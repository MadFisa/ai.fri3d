...
m = np.logical_and(
    data_mag['Epoch'] >= datetime(2008, 12, 17, 3, 30),
    data_mag['Epoch'] <= datetime(2008, 12, 17, 14, 30),
)
d = data_mag['Epoch'][m]
...
dfr, profiles = fit2insitu(
    ...
    toroidal_height=PolyProfile(bounds=[
        u.Unit('km/s').to(u.Unit('m/s'), [300, 500]).tolist(),
        u.au.to(u.m, [0.8, 1.2]).tolist()
    ]),
    poloidal_height=PolyProfile(bounds=[
        u.au.to(u.m, [0.01, 0.2]).tolist()
    ]),
