"""Example of the magnetic field lines of the FRi3D model."""
...
def example_line(
        ...
        polarity=1.0,
        chirality=1.0):
    """Plot magnetic field lines of the FRi3D model."""
    fr = FRi3D(
        latitude=latitude,
        longitude=longitude,
    ...
    for _ in range(20):
        ...
        x, y, z, b = fr.line(r, phi, s=np.linspace(0.1, 0.9, 100))
        x *= u.m.to(u.au)
        y *= u.m.to(u.au)
        z *= u.m.to(u.au)
        b *= u.T.to(u.nT)
    ...
    def orthogonal_proj(zfront, zback):
        """Define orthogonal projection for a 3D plot."""
